    ...
    <main className="min-h-screen flex items-center justify-center px-6 bg-[#050816]">
      ...
      >
        ...
        <div className="rounded-2xl bg-white/[0.03] border border-white/10 p-8 backdrop-blur-xl">
          <h1 className="font-display text-3xl font-semibold tracking-tight mb-2">Создать аккаунт</h1>
          <p className="text-sm text-white/50 mb-8">Бесплатно. 10 минут на пробу.</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[12px] uppercase tracking-wider text-white/40 mb-1.5">Имя</label>
              <input
                type="text"
                required
                value={name}
            ...
            >
              {loading ? <Loader2 className="size-4 animate-spin" /> : <>Создать аккаунт <ArrowRight className="size-4 group-hover:translate-x-0.5 transition" /></>}
            </button>
          </form>

          <p className="text-center text-[13px] text-white/40 mt-6">
            Уже есть аккаунт? <Link href="/login" className="text-white/80 hover:text-white">Войти</Link>
          </p>
        </div>
      </motion.div>
    </main>
